...
async function main() {
  ...
  const oracle = new ethers.Contract(ORACLE, ["function addTvlSource(address source)"], ethers.provider);
  ...
  const data = oracle.interface.encodeFunctionData("addTvlSource", [STAKING]);
  
  const value = {
    to: ORACLE, value: "0", data, operation: 0,
    safeTxGas: 200000, baseGas: 0, gasPrice: 0,
  ...
  // Send with high gas limit — Safe pays for its own execution
  ...
  // Verify
  const oracle2 = new ethers.Contract(ORACLE, ["function getTvlSourceCount() view returns (uint256)"], ethers.provider);

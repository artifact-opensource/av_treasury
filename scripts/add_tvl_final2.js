...
async function main() {
  ...
  // Safe pays gas from its own balance (0.00158 ETH)
  // At ~1.5 gwei, that's enough for ~100k+ gas
  ...
  // Verify
  const oracleAbi = ["function getTvlSourceCount() view returns (uint256)", "function tvlSources(uint256) view returns (address)"];
  const oracle = new ethers.Contract(NEW_ORACLE, oracleAbi, ethers.provider);
  const count = await oracle.getTvlSourceCount();
  console.log("TVL sources:", count.toString());

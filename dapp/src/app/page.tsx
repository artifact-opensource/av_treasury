...
export default function DashboardPage() {
  ...
  const { au: auOracle, ag: agOracle } = useOraclePrices()
  ...
  const [showTrending, setShowTrending] = useState(false)
    ...
    const filteredTokens = useMemo(() => {
      ...
      // Override Au and Ag prices from Oracle if available
      const marketsWithOracle = marketData.map(token => {
        if (token.symbol === 'AU' || token.symbol === 'au' || token.id === 'artifact-utility') {
          if (auOracle?.price) {
            // Assuming oracle price is in 18 decimals or similar, 
            // we need to know the oracle's precision. 
            // Usually, it's 10^18 or 10^8. Let's assume 10^18 for now and adjust if needed.
            return { ...token, current_price: Number(auOracle.price) / 1e18 }
          }
        }
        if (token.symbol === 'AG' || token.symbol === 'ag' || token.id === 'artifact-governance') {
          if (agOracle?.price) {
            return { ...token, current_price: Number(agOracle.price) / 1e18 }
          }
        }
        return token
      })

      let result = marketsWithOracle.filter(token =>
